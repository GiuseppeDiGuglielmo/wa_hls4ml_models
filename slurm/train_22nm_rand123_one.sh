#!/bin/bash
# One 22nm rand123 config = 4 seeds trained then eval'd on the rand123 test set.
# Env: VARIANT OPT LR EP PAT SRC FREEZE CONFIG
#SBATCH -N 1
#SBATCH -C gpu
...
SPLIT=${SCRATCH}/catapult_22nm/rand123
RES=${SCRATCH}/catapult_22nm/eval_results_rand123
mkdir -p "${RES}"

VARIANT=${VARIANT:-plain}; OPT=${OPT:-adamw}; LR=${LR:-1e-3}; EP=${EP:-200}; PAT=${PAT:-30}
SRC=${SRC:-none}; FREEZE=${FREEZE:-0}; CONFIG=${CONFIG:?}
...
for seed in a b c d; do
  tag="r123_${CONFIG}_${seed}"
  echo "===== TRAIN ${VARIANT} ${tag}  (src=$(basename ${SRC:-none}) freeze=${FREEZE}) ====="
...
echo "DONE_RAND123 ${VARIANT} ${CONFIG}"
